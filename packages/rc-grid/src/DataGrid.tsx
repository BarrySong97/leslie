...
function DataGrid<R>(props: DataGridProps<R>) {
        ...
        filterRows.some((row, index) => {
            ...
            domRows.push(
                <DataGridRow<R>
                    ...
                    rowIndexCode="row"
                    rowIndex={index}
                    isExpanded={state.expandableTreeKey.includes(row.key)}
                    width={width}
                    isRowLast={[false]}
                    level={0}
                    scrollWidth={scrollWidth}
            ...
            const renderChildrenRows = (
                ...
                // 计算表格树
                if (
                    tree?.childrenColumnName &&
                    state.expandableTreeKey.includes(parent.key)
                ) {
                    const childrenRows = tree?.onChildrenRows?.(parent) || []

                    childrenRows.forEach((child, childIndex) => {
                        const isRowLastList = [
                            ...isRowLast,
                            childIndex === childrenRows.length - 1,
                        ]
                        const tempStyle = {
                        ...
                        const tempRow = (
                            <DataGridRow<R>
                                ...
                                level={level}
                                isExpanded={state.expandableTreeKey.includes(
                                    child.key
                                )}
                                isRowLast={isRowLastList}
                                width={width}
                                scrollWidth={scrollWidth}
                                scrollLeft={scrollLeft}
                                styled={tempStyle}
                        ...
                        domRows.push(tempRow)
                        top += child.height
                        renderExpandedRowRender(child)
                        renderChildrenRows(
                            child,
                            level + (tree?.indentSize || 1),
                            isRowLastList
                        )
                    })
                }
            ...
            renderChildrenRows(row, 1, [false])
    ...
    const onScrollDebounce = debounce(
        ({ target }: React.UIEvent<HTMLDivElement, UIEvent>) => {
            ...
            if (currentTarget) {
                if (
                    ...
                    calcCacheRemove -
                        (estimatedColumnWidth * cacheRemoveCount) / 10
                ) {
